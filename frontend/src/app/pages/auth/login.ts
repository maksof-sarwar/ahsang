...
@Component({
    selector: 'app-login',
    standalone: true,
    providers: [MessageService, ApiClientService],
                ...
                <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                    <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                        ...
                        <div>
                            ...
                            <p-button label="Sign In" styleClass="w-full" (click)="onSubmit($event)"></p-button>
                            <div class="flex items-center justify-center mt-4">
                                <a routerLink="/auth/register" class="font-medium text-center no-underline cursor-pointer text-primary">Don't have an account?</a>
                            </div>
                        </div>
                    </div>
                </div>
...
export class Login {
    constructor(private router: Router, private messageService: MessageService, private apiClientService: ApiClientService) { }
    email: string = '';
    ...
    onSubmit(e: Event) {
        e.preventDefault();
        const r = this.apiClientService.fetcher.path('/auth/login').method('post').create()
        ...
        from(r({
            ...
            .subscribe({
                ...
                error: (err) => {
                    this.messageService.add({ severity: 'error', summary: 'Error', detail: err.data?.error?.message });
                }
            })
    }
}
